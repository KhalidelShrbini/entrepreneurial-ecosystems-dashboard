-- Ecosystem Readiness scoring view.
-- Weighted, min-max-normalized composite across six factors. Fragile/conflict-affected
-- status is exposed as a filterable flag on the underlying table, not scored down here --
-- fragile contexts are a deliberate target segment for entrepreneurship support
-- programmes, not a disqualifier.
...
WITH bounds AS (
    SELECT
        ...
        MIN(business_reg_rating) AS min_reg, MAX(business_reg_rating) AS max_reg,
...
scored AS (
    SELECT
        ...
        (s.business_reg_rating - b.min_reg) * 1.0 / NULLIF(b.max_reg - b.min_reg, 0) AS n_ease,
...
SELECT
    ...
    self_employed_pct, agriculture_pct_gdp, poverty_headcount_pct, business_reg_rating,
