...
def build_insights(df: pd.DataFrame) -> list[str]:
    ...
    if d["days_to_start_business"].notna().sum() >= 5:
        fast = d.dropna(subset=["days_to_start_business"]).sort_values("days_to_start_business").iloc[0]
        slow = d.dropna(subset=["days_to_start_business"]).sort_values("days_to_start_business", ascending=False).iloc[0]
        insights.append(
            f"Business registration friction varies sharply: <b>{fast['country_name']}</b> takes as little as "
            f"{fast['days_to_start_business']:.0f} days to start a business, versus {slow['days_to_start_business']:.0f} "
            f"days in <b>{slow['country_name']}</b>."
        )

    corr_cols = ["self_employed_pct", "internet_users_pct", "gdp_growth_pct", "days_to_start_business"]
...
def build_recommendations(df: pd.DataFrame) -> list[str]:
    ...
    if d["days_to_start_business"].notna().sum() >= 5:
        friction = d.dropna(subset=["days_to_start_business"])
        high_friction = friction[friction["days_to_start_business"] > friction["days_to_start_business"].median()]
        high_friction_high_potential = high_friction[
            high_friction["self_employed_pct"] > high_friction["self_employed_pct"].median()
        ] if high_friction["self_employed_pct"].notna().any() else pd.DataFrame()
        if not high_friction_high_potential.empty:
            names = high_friction_high_potential.sort_values("ecosystem_readiness_score", ascending=False).head(3)["country_name"].tolist()
            recs.append(f"<b>Regulatory-support opportunity:</b> {', '.join(names)} combine above-median entrepreneurial "
                        f"activity with above-median registration friction -- a training/advocacy programme on formalization "
                        f"could unlock latent activity here.")
...
with tab_readiness:
    ...
    with col_a:
        st.subheader("Opportunity vs. friction quadrant")
        st.markdown('<p class="section-note">Self-employment rate (entrepreneurial activity) vs. days to register a '
                    'business (friction). Top-left: high activity despite friction -- often the strongest case for support.</p>',
                    unsafe_allow_html=True)
        qd = filtered.dropna(subset=["self_employed_pct", "days_to_start_business"])
        if not qd.empty:
            x_med, y_med = qd["days_to_start_business"].median(), qd["self_employed_pct"].median()
            fig_q = px.scatter(qd, x="days_to_start_business", y="self_employed_pct", color="region",
                                size="population", hover_name="country_name", size_max=45,
                                labels={"days_to_start_business": "Days to start a business", "self_employed_pct": "Self-employment (%)"})
    ...
    with col_b:
        ...
        corr_cols = {
            ...
            "self_employed_pct": "Self-employment", "days_to_start_business": "Days to start biz",
...
with tab_regional:
    ...
    metric_options = {
        ...
        "Internet users (%)": "internet_users_pct", "Days to start a business": "days_to_start_business",
...
with tab_data:
    ...
    display_cols = ["country_name", "region", "income_level", "is_fragile_or_conflict",
                     ...
                     "days_to_start_business", "agriculture_pct_gdp", "poverty_headcount_pct",
...
- **Readiness score:** min-max normalized, weighted composite of GDP growth, internet penetration, self-employment
  rate, business-registration speed, inflation, and unemployment. Weights are adjustable in the sidebar and default
...
- **Limitation:** self-employment and business-registration-speed indicators have partial country coverage in the
  underlying data; rankings should be treated as a screening layer, not a standalone decision input.
""")
