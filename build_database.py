...
scored AS (
    ...
    WHERE s.income_level NOT IN ('High income')
      ...
      AND s.internet_users_pct IS NOT NULL
)
SELECT
    ...
    ROUND(
        COALESCE(n_growth, 0) * 0.30 +
        COALESCE(n_digital, 0) * 0.25 +
        COALESCE(n_popgrowth, 0) * 0.15 +
        COALESCE(n_stability, 0) * 0.20 +
        COALESCE(n_labor, 0) * 0.10
